    ...
    def run_acquisition(self, acq, acq_list):
        ...
        for i in range(steps):
            if self.stopflag is True:
                ...
                # check and implement for oblique scanning

                if (self.state['scanning_oblique_m45'] is True):
                    z_vals = acq.get_zstep()
                    z_stepping = z_vals[0]
                    z_starting = z_vals[1]
                    z_ending = z_vals[2]

                    if z_ending >= z_starting:
                        x_rel = abs(z_stepping)
                        move_dict.update({'x_rel':x_rel}) # update dict to perform x shift

                    if z_ending < z_starting:
                        x_rel = -abs(z_stepping)
                        move_dict.update({'x_rel':x_rel})


                if (self.state['scanning_oblique_p45'] is True):
                    z_vals = acq.get_zstep()
                    z_stepping = z_vals[0]
                    z_starting = z_vals[1]
                    z_ending = z_vals[2]
                    ...
                    if z_ending >= z_starting:
                        print('first z condition satisfied')
                        x_rel = -abs(z_stepping)
                        move_dict.update({'x_rel':x_rel})

                    if z_ending < z_starting:
                        print('second z condition satisfied')
                        x_rel = abs(z_stepping)
                        move_dict.update({'x_rel':x_rel})

                # if normal scan is true, clearing x shift from dict (redundant?)

                if (self.state['scanning_normal'] is True):
                    move_dict.pop('x_rel', None)
