...
class mesoSPIM_MainWindow(QtWidgets.QMainWindow):
    ...
    def initialize_and_connect_widgets(self):
        ...
        # check state of radio buttons for radio buttons
        self.p45deg.toggled.connect(self.oblique_scan_check)
        self.m45deg.toggled.connect(self.oblique_scan_check)
        self.NormScan.toggled.connect(self.oblique_scan_check) # default selection on opening gui
    ...
    @QtCore.pyqtSlot()
    def oblique_scan_check(self):
        self.state['scanning_oblique_p45'] = self.p45deg.isChecked() # update all scan states with each selection
        self.state['scanning_oblique_m45'] = self.m45deg.isChecked()
        self.state['scanning_normal'] = self.NormScan.isChecked()
         # will print twice as two radio button states
        if self.p45deg.isChecked() is True:
            print('Scan state changed to +45 oblique scan') # prints twice as two radio buttons are toggled '_'
        if self.m45deg.isChecked() is True:
            print('Scan state changed to -45 oblique scan')
        if self.NormScan.isChecked() is True:
            print('Scan state changed to normal z scan')
